...
export default function ServiceSelection() {
  ...
  const grouped = services.reduce((acc, svc) => {
    ...
    const icon = svc.service_categories?.icon || 'Sparkles'
  ...
  return (
    <div style={{ minHeight:'100vh', background:'#0B0D17', fontFamily:"'Outfit',sans-serif", color:'#fff' }}>
      <div style={{ display:'flex',alignItems:'center',justifyContent:'space-between',padding:'18px 22px 0' }}>
        <button onClick={() => navigate('/quote/details')}
      ...
      <div style={{ padding:'28px 22px 160px' }}>
        ...
        {loading ? (
          ...
          <div style={{ textAlign:'center',padding:'40px 20px',color:'rgba(255,255,255,.3)' }}>
            <div style={{ marginBottom:10 }}><CategoryIcon name="Sparkles" size={32} color="rgba(255,255,255,.3)" /></div>
          ...
          Object.entries(grouped).map(([cat, { icon, items }]) => (
            <div key={cat}>
              ...
              {/* En-tête catégorie avec vraie icône */}
              <div style={{ display:'flex',alignItems:'center',gap:8,fontSize:11,fontWeight:700,color:'rgba(255,255,255,.5)',textTransform:'uppercase',letterSpacing:'.1em',margin:'24px 0 10px' }}>
                <div style={{ width:28,height:28,borderRadius:8,background:'rgba(255,77,46,.1)',border:'1px solid rgba(255,77,46,.2)',display:'flex',alignItems:'center',justifyContent:'center' }}>
                  <CategoryIcon name={icon} size={14} color="#FF4D2E" />
                </div>
                {cat}
                <span style={{ fontSize:10,color:'rgba(255,255,255,.2)',fontWeight:400,textTransform:'none' }}>
                ...
                return (
                  <div key={svc.id} onClick={() => toggle(svc)}
                    ...
                    <div style={{ textAlign:'right',flexShrink:0 }}>
                      <div style={{ fontSize:12,fontWeight:600,color:'#FF4D2E' }}>
                        {formatPrice(svc.price)}/{svc.unit}
                      </div>
                      {isSel && (
                        <div style={{ fontSize:10,color:'rgba(255,77,46,.8)',marginTop:2 }}>
        ...
        {selected.length > 0 && (
          <div style={{ background:'#111827',borderRadius:12,padding:'10px 14px',marginBottom:12 }}>
            ...
            <div style={{ display:'flex',justifyContent:'space-between',fontSize:16,fontWeight:700 }}>
              <span>Total TTC</span>
              <span style={{ color:'#F59E0B' }}>{formatPrice(totalTTC)}</span>
            </div>
          </div>
        )}
